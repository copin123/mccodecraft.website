<div class="starter-template">
    <form class="form-horizontal" role="form" id='article-create-form' method='POST'
        <#if article??>action="/article/update/:id"<#else>action="/article/create"
        </#if>>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-5">
                ...
                <#if article??>value="${article.getSummary()}" </#if> />
            </div>
        </div>
        <#if article??>
    ...
    <textarea class="form-control" name='article-content' id="content" rows='4' cols='50' form='article-create-form'
          placeholder="Enter article content"><#if article??>${article.getContent()}</#if>
    </textarea>
